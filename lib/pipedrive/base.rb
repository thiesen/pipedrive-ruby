...
module Pipedrive
  ...
  class Base < OpenStruct
    ...
    class << self
      ...
      def all(api_token, get_absolutely_all = true, response = nil, options={})
        path = "#{resource_path}?api_token=#{api_token}"
      ...
      def find(id, api_token)
        path = "#{resource_path}/#{id}?api_token=#{api_token}"
        res = get path
        res.ok? ? new(res) : bad_response(res,id)
      end

      def find_by_name(name, api_token, opts={})
        opts.merge!({:api_token => api_token})
